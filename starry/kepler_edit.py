...
class System(object):
    ...
    def design_matrix_input(self, t):
        inputs = [self._math.reshape(self._math.to_array_or_tensor(t), [-1])
            * self._time_factor,
            self._primary._r,
            self._primary._m,
            self._primary._prot,
    ...
    def design_matrix(self, t):
        ...
        """
        inputs = self.design_matrix_input(t)
        return self.ops.X(*inputs)
    ...
    def flux(self, t, total=True, integrated=False):
        ...
        if total:
            ...
            if integrated and self.primary.map.nw is not None:
                ...
                ay = np.sum(ay, axis=1)
            # return self._math.dot(X, ay)
            print("system: ", X.shape, ay.shape, (np.dot(X,ay)).shape)
            print("y", ay)
            print(X[0], X[0].shape)
            # return np.dot(X, ay)
        else:
